...
[prompt optimization guide](references/prompt-optimization.md).

### 2. Prefix caching optimization

If the prompt is more than 200 words, implement the [prefix caching API](references/prefix-caching.md).

### 3. Lifecycle and best practices

- The model must be fully downloaded and available before calling the first inference. Follow the guide on [implementing a generative model](references/get-started.md) to check that the `FeatureStatus` of a model is `AVAILABLE` before making an inference.
...
1. **Check for API availability:** Verify Structured Output feature is available on the device with `isStructuredOutputFeatureAvailable()` before using it. Refer to the [Structured Output API guide](references/structured-output.md) for full instructions.
...
3. **Example:**
   ...
   After using Structured Output API:
   ...
   suspend fun parseEmail(email: String): ParsedEmail? {
       val parseEmailPrompt = "Parse this email: $email"
